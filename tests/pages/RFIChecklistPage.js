...
class RFIChecklistPage extends BasePage {
  ...
  // getVisibleCode is INHERITED from BasePage rather than duplicated here.
  //
  // The identical breadcrumb implementation used to exist in BOTH classes, which
  // meant the mobile fallback had to be written twice or — as actually happened —
  // only helped whichever copy the caller happened to go through. BasePage's
  // version documents the breadcrumb mechanics and the mobile fallback (mobile
  // renders no breadcrumb at all, and shows the code CSS-truncated in the header).
